...
import {
  groupQualitativeByCategory,
  computeQualitativeGlobalMetrics,
  collectObservations,
...
export default function QualitativeDashboard () {
  const { qualitative, qualitativeLoading, qualitativeError, loadQualitative, clearAllData } = useAppData();
  const rows = qualitative?.rows ?? [];
  const questions = qualitative?.questions ?? [];
  const isLoaded = qualitative !== null;
  const fileName = qualitative?.fileName ?? "";
  ...
  const categoryGroups = useMemo(
    () => groupQualitativeByCategory(questions, deferredRows),
  ...
  const globalMetrics = useMemo(
    () => computeQualitativeGlobalMetrics(displayedGroups, deferredRows.length),
    [displayedGroups, deferredRows.length]
  );

  const observations = useMemo(
    () => collectObservations(deferredRows),
    [deferredRows]
  );
  ...
  const handleExportPdf = useCallback(() => {
    ...
    try {
      ...
      }, `auditoria-qualitativa-${format(new Date(), "dd-MM-yyyy")}.pdf`, observations);
  ...
  }, [displayedGroups, startDate, endDate, selectedSector, selectedCategory, filteredRows.length, observations]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-6">
        <div className="w-full max-w-lg space-y-6 text-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Auditoria Qualitativa</h1>
            <p className="mt-2 text-muted-foreground">
              Faça upload do arquivo CSV de Auditoria Qualitativa para começar a análise
            </p>
          </div>
          <CsvUpload
            onFileLoaded={loadQualitative}
            isLoading={qualitativeLoading}
            fileName={fileName}
            error={qualitativeError}
          />
        </div>
      </div>
    );
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/80 backdrop-blur-sm">
        <div className="flex flex-wrap items-center gap-4 px-4 py-3 lg:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-foreground">Auditoria Qualitativa</h1>
          </div>
